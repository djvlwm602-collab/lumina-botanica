...
export default function ThemeSelector({ currentBg, onSelect }: ThemeSelectorProps) {
  return (
    <motion.div
      ...
      className="fixed bottom-6 right-6 z-[20] flex flex-col sm:flex-row items-center gap-2 p-2 liquid-glass"
      style={{
        borderRadius: '9999px',
      ...
      {THEMES.map((theme) => {
        ...
        return (
          <button
            ...
            className={`relative p-3 rounded-full flex items-center justify-center transition-all duration-300 ${isActive ? 'bg-white/30 scale-110 shadow-sm' : 'hover:bg-white/15 hover:scale-105 opacity-70 hover:opacity-100'}`}
          >
